...
class EpisodeMemory:
    ...
    def sample(self, n, L):
        ...
        datas = [self.episodes[i].sample(L) for i in indeces]
        return datas

    def sample2(self, n, L):
        indeces = np.random.randint(0, len(self.episodes), n)
        ...
        datas_rewards      = rnn.pad_sequence([torch.tensor(data[2]) for data in datas], batch_first=True).float()
        datas_dones        = rnn.pad_sequence([torch.tensor(data[3]) for data in datas], batch_first=True).float()

        return datas_observations, datas_actions, datas_rewards, datas_dones

    def sample3(self, n, L):
        indeces = np.random.randint(0, len(self.episodes), n)
        ...
        datas_observations2 = rnn.pad_sequence([torch.tensor(data[0][1:]) for data in datas], batch_first=True).float()
        ...
        return datas_observations, datas_actions, datas_rewards, datas_dones, datas_observations2

    def sample2_obs(self, n, L):
        indeces = np.random.randint(0, len(self.episodes), n)

        # datas = [self.episodes[i].sample(L)[0] for i in indeces]
        # datas_observations = rnn.pad_sequence([torch.tensor(data) for data in datas], batch_first=True).float()

        datas_observations = rnn.pad_sequence([torch.tensor(self.episodes[i].sample(L)[0]) for i in indeces], batch_first=True).float()
        
        return datas_observations
